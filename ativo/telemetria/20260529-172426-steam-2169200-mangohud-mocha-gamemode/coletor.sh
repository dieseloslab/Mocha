...
gm_state_now() {
  ...
  if printf '%s' "$raw" | grep -Eiq 'inactive|not active|off|desativ'; then
    printf 'OFF\t%s' "$raw"
...
gpu_now() {
  ...
  pstate="$(printf '%s' "${pstate:-NA}" | sed 's/^ *//;s/ *$//')"
  gpu_util="$(printf '%s' "${gpu_util:-NA}" | sed 's/^ *//;s/ *$//')"
  mem_util="$(printf '%s' "${mem_util:-NA}" | sed 's/^ *//;s/ *$//')"
  temp="$(printf '%s' "${temp:-NA}" | sed 's/^ *//;s/ *$//')"
  power="$(printf '%s' "${power:-NA}" | sed 's/^ *//;s/ *$//')"
  power_limit="$(printf '%s' "${power_limit:-NA}" | sed 's/^ *//;s/ *$//')"
  gr_clock="$(printf '%s' "${gr_clock:-NA}" | sed 's/^ *//;s/ *$//')"
  mem_clock="$(printf '%s' "${mem_clock:-NA}" | sed 's/^ *//;s/ *$//')"
...
echo "Coletor iniciado em $(date --iso-8601=seconds)" >> "$EVENTS"
  ...
  if [ $((i % 5)) -eq 0 ]; then
    {
      echo
      echo "===== $iso ====="
      pgrep -af 'steamapps|Proton|proton|wine64|wine|wineserver|pressure-vessel|GameOverlayUI|gamemoderun|mangohud' 2>/dev/null || echo "Nenhum processo detectado."
    } >> "$PROCS"
  fi
...
echo "Coletor encerrado em $(date --iso-8601=seconds)" >> "$EVENTS"
