...
if printf '%s' "$KERNEL" | grep -Eqi '(^|[-.])rc[0-9]*($|[-.])'; then
...
if have nvidia-smi; then
    ...
    nvidia-smi -q -d PERFORMANCE,POWER,CLOCK,TEMPERATURE,PCI 2>/dev/null || true
...
GPU_BDF="$(lspci -Dnn 2>/dev/null | awk '/VGA compatible controller|3D controller|Display controller/ && /NVIDIA/{print $1; exit}')"
...
ZRAM_LINE="$(swapon --show --noheadings --raw --output=NAME,PRIO 2>/dev/null | awk '$1 ~ /zram/ {print; exit}')"
if [ -n "$ZRAM_LINE" ]; then
...
printf '%s' "$THP" | grep -q '\[madvise\]' && pass "THP está em madvise." || fail "THP não está em madvise."
...
if sudo journalctl -k -b --no-pager 2>/dev/null | grep -Eqi 'NVRM: Xid'; then
    fail "O boot atual contém erro NVIDIA Xid."
else
    pass "Nenhum NVIDIA Xid foi encontrado no boot atual."
fi

if sudo journalctl -k -b --no-pager 2>/dev/null | grep -Eqi 'PCIe Bus Error|AER:.*error'; then
