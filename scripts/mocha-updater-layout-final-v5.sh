...
[ -f src/main.rs ] && cp -a src/main.rs "src/main.rs.bak-layout-final-v5-$STAMP"
...
<!DOCTYPE policyconfig PUBLIC
 ...
 "http://www.freedesktop.org/standards/PolicyKit Policy Configuration 1.0//EN">
<policyconfig>
  <vendor>Mocha</vendor>
...
impl Lang {
    ...
    fn t(self, key: &'static str) -> &'static str {
        match self {
            Lang::Pt => match key {
                "subtitle" => "Atualização real sem terminal. Clique, autorize e acompanhe no próprio programa.",
                ...
                "about_body" => "Mocha Updater em modo real sem terminal. As ações rodam por Polkit em segundo plano, com progresso na interface.",
                "auth_hint" => "Quando necessário, o sistema abre a autorização Polkit para senha administrativa.",
                _ => Lang::En.t(key),
            },
            Lang::En => match key {
                "subtitle" => "Real updates without terminal. Click, authorize and follow progress inside the app.",
                ...
                "backup_body" => "Real snapshots are written to /var/lib/mocha-updater/snapshots before system update or kernel changes.",
                "settings_body" => "Supported languages: Portuguese, English, French and Spanish. Other locales fall back to English.",
                "about_body" => "Real Mocha Updater without terminal. Actions run through Polkit in the background, with progress inside the interface.",
                "auth_hint" => "When needed, the system opens Polkit authorization for the administrator password.",
                _ => key,
            },
            Lang::Fr => match key {
                ...
                "kernel_body" => "Installe uniquement les paquets déjà disponibles dans les dépôts configurés.",
                ...
                "about_body" => "Mocha Updater réel sans terminal. Actions en arrière-plan via Polkit.",
                "auth_hint" => "Si nécessaire, le système ouvre l'autorisation Polkit.",
                _ => Lang::En.t(key),
            },
            Lang::Es => match key {
                ...
                "apply_kernel_sub" => "Instalar recomendado",
                "kernel_body" => "Instala solo paquetes existentes en los repositorios configurados.",
                ...
                "about_body" => "Mocha Updater real sin terminal. Acciones en segundo plano vía Polkit.",
                "auth_hint" => "Si es necesario, el sistema abre autorización Polkit.",
                _ => Lang::En.t(key),
            },
        }
    }
...
impl MochaUpdater {
    ...
    fn start_action(&mut self, action: &str) {
        ...
        self.log(format!("{}: {}", self.lang.t("running"), safe));
        ...
        egui::TopBottomPanel::top("top")
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        let state = if let Some(runner) = &self.runner {
                            ...
                            self.lang.t("ready").to_string()
                        } else {
                            self.last_result.clone()
                        };

                        ui.label(RichText::new(state).size(14.0).color(mocha_text()).strong());
                    });
                });
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    tab_button(ui, &mut self.tab, Tab::Logs, self.lang.t("logs"));
...
impl MochaUpdater {
    fn ui_general(&mut self, ui: &mut egui::Ui) {
        card(ui, ui.available_width(), |ui| self.system_card(ui));

        ui.add_space(12.0);

        card(ui, ui.available_width(), |ui| self.status_card(ui));
    ...
    fn system_card(&self, ui: &mut egui::Ui) {
        ...
        kv(ui, self.lang.t("cpu"), &self.info.cpu);
        kv(ui, self.lang.t("gpu"), &self.info.gpu);
        kv(ui, self.lang.t("current_kernel"), &self.info.kernel);
        kv(ui, self.lang.t("driver"), &self.info.driver);
        kv(ui, self.lang.t("distro"), &self.info.distro);
        kv(ui, self.lang.t("arch"), &self.info.arch);
        kv(ui, self.lang.t("hostname"), &self.info.hostname);
        kv(ui, self.lang.t("uptime"), &self.info.uptime);
    }

    fn status_card(&self, ui: &mut egui::Ui) {
        title(ui, self.lang.t("status"));

        kv(ui, self.lang.t("pacman"), &format!("{} {}", self.info.pacman_pending, self.lang.t("pending")));
        kv(ui, self.lang.t("flatpak"), &format!("{} {}", self.info.flatpak_pending, self.lang.t("pending")));
        kv(ui, self.lang.t("recommendation"), &self.info.recommendation);

        ui.add_space(8.0);
        ui.label(RichText::new("Logs: /var/log/mocha-updater").color(mocha_orange()).size(13.0));
        ui.label(RichText::new("Snapshots: /var/lib/mocha-updater/snapshots").color(mocha_orange()).size(13.0));
        ui.label(RichText::new(self.lang.t("auth_hint")).color(mocha_muted()).size(13.0));
    }
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for line in self.logs.iter().skip(start) {
                        ui.label(RichText::new(wrap_words(line, 150, 120)).monospace().size(12.5).color(mocha_text()));
                    }
                });
        });
    }
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for line in &self.logs {
                        ui.label(RichText::new(wrap_words(line, 200, 130)).monospace().size(12.5).color(mocha_text()));
                    }
                });
        });
    }
...
fn tab_button(ui: &mut egui::Ui, current: &mut Tab, tab: Tab, label: &str) {
    ...
    if ui.add_sized([158.0, 34.0], button).clicked() {
        *current = tab;
    }
}
...
fn kv(ui: &mut egui::Ui, key: &str, value: &str) {
    ...
    let line_len = ((available - 190.0) / 7.2).clamp(32.0, 120.0) as usize;
    let value = wrap_words(value, 260, line_len);

    ui.horizontal_top(|ui| {
        ui.add_sized(
            [160.0, 20.0],
            egui::Label::new(RichText::new(format!("{}:", key)).color(mocha_orange()).size(13.5).strong()),
        );

        ui.label(RichText::new(value).color(mocha_text()).size(13.5));
    });
...
fn action_button(ui: &mut egui::Ui, title: &str, sub: &str, width: f32, primary: bool, disabled: bool) -> bool {
    ...
    if disabled {
        ui.add_sized([width, 72.0], button);
        false
    } else {
        ui.add_sized([width, 72.0], button).clicked()
    }
}
...
## Mocha Updater layout final V5 — $STAMP

Script canônico:
- $ROOT/scripts/mocha-updater-layout-final-v5.sh
...
ok "Mocha Updater layout final V5 compilado"
...
ok "Script canônico: $ROOT/scripts/mocha-updater-layout-final-v5.sh"

pkill -f '/usr/local/bin/mocha-updater' >/dev/null 2>&1 || true
nohup /usr/local/bin/mocha-updater >/tmp/mocha-updater-layout-final-v5.log 2>&1 &
